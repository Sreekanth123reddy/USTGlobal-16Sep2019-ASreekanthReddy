package com.ustglobal.map;

import java.util.ArrayList;
import java.util.HashMap;
import java.util.Iterator;

public class TestF {
public static void main(String[] args) {
	Student s1 = new Student(2, "divya", 67.89);
	Student s2 = new Student(1, "Pujitha", 95.3);
	Student s3  = new Student(4,"Vaziha",87.4);
	Student s4 = new Student(9,"sreekanth",56.85);
	Student s5 = new Student(7,"Amrutha",56.85);
	Student s6 = new Student(5,"Zpoomark",68.9);
	Student s7 = new Student(3,"Pranay",72.9);
	Student s8 = new Student(8,"Mandaara",99.99);
	Student s9 = new Student(6,"Imaran",35.9);
	Student s10 = new Student(10,"NesaRani",85.45);
	Student s11 = new Student(11,"sreenath",45.9);
	
	
	ArrayList<Student> al = new ArrayList<>();
	
	al.add(s1);
	al.add(s2);
	al.add(s3);
	al.add(s4);
	
	ArrayList<Student> al1= new ArrayList<Student>();
	al1.add(s5);
	al1.add(s6);
	al1.add(s7);
	
	ArrayList<Student> al2 = new ArrayList<Student>();
	al2.add(s9);
	al2.add(s10);
	al2.add(s11);
	
	HashMap<String,ArrayList<Student>> hm = new HashMap<>();
	
	hm.put("first", al);
	hm.put("Second", al1);
	hm.put("third", al2);
	
	ArrayList<Student> first = hm.get("Second");
	
	
	Iterator<Student> it = first.iterator();
	while(it.hasNext())
	{
		Student s = it.next();
		System.out.println("Id is "+s.id);
		System.out.println("Name is "+s.name);
		System.out.println("Percentage is "+s.percentage);
		System.out.println("===========");
	}
}
}
